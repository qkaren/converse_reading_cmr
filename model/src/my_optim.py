...
class EMA:
    ...
    def __init__(self, model, gamma=0.99):
        self.gamma = gamma
        self.shadow = deepcopy(list(p.data.new(p.size()).zero_() for p in model.parameters()))

    def update(self, parameters):
        gamma = self.gamma
        for p, avg in zip(parameters, self.shadow):
            avg.mul_(gamma).add_(p.mul(1 - gamma))

    def copy_out(self):
        return self.shadow

    def dump(model):
        parameters = deepcopy(list(p.data for p in model.parameters()))
        return parameters

    def reset(model, parameters):
        model_p = list(p.data for p in model.parameters())
        for m, p in zip(model_p, parameters):
            m = deepcopy(p)
